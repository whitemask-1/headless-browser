...
def launch_control_panel(sites):
    ...
    for color in [DOT, "#22d3ee", "#a78bfa"]:
        d = tk.Label(dot_row, text="●", fg=color, bg=BG, font=("Helvetica", 10), cursor="hand2")
    ...
    title = tk.Label(header, text="⠿ Sites", fg=TEXT, bg=BG, font=("Helvetica", 11, "bold"), cursor="fleur")
    title.pack(side="left")
    title.bind("<ButtonPress-1>", on_drag_start)
    title.bind("<B1-Motion>", on_drag_motion)

    # minimize button
    min_btn = tk.Label(header, text="—", fg=SUBTEXT, bg=BG, font=("Helvetica", 11), cursor="hand2")
    min_btn.pack(side="right", padx=(8, 0))
    min_btn.bind("<Button-1>", lambda e: toggle_minimize())

    # close button
    close_btn = tk.Label(header, text="✕", fg=SUBTEXT, bg=BG, font=("Helvetica", 11), cursor="hand2")
    ...
    for key, url in sites.items():
        ...
        num = tk.Label(row, text=f" {key}", fg=ACCENT, bg=BTN_BG, font=("Helvetica", 10, "bold"), width=2)
        num.pack(side="left", padx=(6, 0), pady=6)

        btn = tk.Label(row, text=label, fg=TEXT, bg=BTN_BG, font=("Helvetica", 10), anchor="w", cursor="hand2")
    ...
    # footer
    footer = tk.Label(panel_frame, text="drag to move  •  — to minimize", fg=SUBTEXT, bg=BG, font=("Helvetica", 8))
    footer.pack()
